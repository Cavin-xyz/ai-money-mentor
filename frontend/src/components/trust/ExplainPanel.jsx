    ...
    <div className="glass-card overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2.5">
          ...
          <span>
            <span className="block text-sm font-bold text-navy-900">{t('explain.title')}</span>
            <span className="block text-[11px] text-navy-900/45">{t('explain.sub', { n: calculations.length })}</span>
          </span>
        </span>
            ...
            <div className="px-5 py-4 space-y-5 max-h-[28rem] overflow-y-auto">
              {assumptions.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {assumptions.map((a) => (
                    <span key={a} className="text-[11px] px-2.5 py-1 rounded-full bg-amber-50 border border-amber-100 text-amber-800">{p(a)}</span>
                  ))}
                </div>
              )}
              {groups.map(([group, rows]) => (
                <div key={group}>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-navy-900/40 mb-2">{p(group)}</p>
                  <table className="w-full text-xs">
                    <tbody>
                      {rows.map((r, i) => (
                        <tr key={i} className="border-b border-navy-900/[0.04] last:border-0 align-top">
                          <td className="py-1.5 pr-3 text-navy-900/75 w-[38%]">{p(r.label)}</td>
